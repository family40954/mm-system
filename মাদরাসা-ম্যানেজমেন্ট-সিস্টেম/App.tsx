...
import { Menu, X, User, GraduationCap, RefreshCw, CheckCircle2 } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // Updated: Class-wise subjects mapping
  const [subjects, setSubjects] = useState<Record<string, string[]>>(() => {
    const saved = localStorage.getItem('madrasa_subjects_map');
  ...
  // --- Auto-Save with Persistence Lock ---
  useEffect(() => {
    ...
    if (!isSyncing && !isLocked) {
      ...
      localStorage.setItem('madrasa_subjects_map', JSON.stringify(subjects));
      localStorage.setItem('madrasa_exam_types', JSON.stringify(examTypes));
    }
  }, [classes, sessions, profile, students, teachers, fees, transactions, incomeCategories, expenseCategories, marksData, subjects, examTypes, isSyncing]);
      ...
      if (current >= 100) {
        ...
        try {
          const keysToClean = [
            ...
            'madrasa_expense_cats', 'madrasa_marks_data', 'madrasa_subjects_map', 'madrasa_exam_types'
          ];
          ...
          localStorage.setItem('madrasa_subjects_map', JSON.stringify(allData.subjects || {}));
  ...
  const renderContent = () => {
    const commonProps = { profile, students, teachers, transactions, fees, marksData, subjects, examTypes, classes, sessions };
    
    switch (currentView) {
      case AppView.DASHBOARD: return <Dashboard {...commonProps} setCurrentView={setCurrentView} />;
      ...
      case AppView.RESULTS: return <Results {...commonProps} setMarksData={setMarksData} setSubjects={setSubjects} setExamTypes={setExamTypes} />;
      ...
      case AppView.SETTINGS: return <Settings profile={profile} setProfile={setProfile} globalData={{...commonProps, incomeCategories, expenseCategories}} onRestore={handleGlobalRestore} />;
      case AppView.BACKUP: return <Backup globalData={{...commonProps, incomeCategories, expenseCategories}} onRestore={handleGlobalRestore} />;
      default: return <Dashboard {...commonProps} setCurrentView={setCurrentView} />;
    }
  };
    ...
    <div className="min-h-screen flex flex-col md:flex-row font-bengali bg-slate-50">
        ...
        <div className="fixed inset-0 z-[2000] bg-madrasa-950/95 backdrop-blur-2xl flex items-center justify-center p-4">
          <div className="bg-white p-14 rounded-[64px] shadow-[0_64px_160px_rgba(0,0,0,0.6)] flex flex-col items-center gap-10 text-center animate-in zoom-in-95 max-w-sm w-full border border-white/20">
            ...
            <div className="space-y-3">
               <h4 className="font-black text-slate-900 text-3xl">
                {syncProgress < 100 ? 'ডাটা সিঙ্ক হচ্ছে' : 'সফল হয়েছে!'}
               </h4>
               <p className="text-slate-500 text-sm font-bold leading-relaxed px-4">
                {syncProgress < 100 
                  ? 'আপনার মাদরাসার সকল তথ্য ডাটাবেজে রিস্টোর করা হচ্ছে। অনুগ্রহ করে ব্রাউজার বন্ধ করবেন না।' 
                  : 'সকল তথ্য সফলভাবে রিস্টোর হয়েছে। সিস্টেম এখন অটোমেটিক রিস্টার্ট হবে।'}
               </p>
            </div>
            {syncProgress === 100 && (
      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-72 bg-madrasa-900 text-white transform transition-transform duration-300 ease-in-out md:relative md:translate-x-0 no-print ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} border-r border-madrasa-800 flex flex-col`}>
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto scrollbar-hide">
          {NAV_ITEMS.map((item) => (
            <button key={item.id} onClick={() => { setCurrentView(item.id); setIsSidebarOpen(false); }} className={`w-full flex items-center gap-4 px-4 py-3 rounded-2xl transition-all duration-300 ${currentView === item.id ? 'bg-madrasa-800 text-white shadow-lg border-l-4 border-gold scale-105' : 'text-madrasa-300 hover:bg-madrasa-800/50 hover:text-white'}`}>
              <span className={currentView === item.id ? 'text-gold' : 'text-madrasa-400'}>{item.icon}</span>
              <span className="font-bold text-sm">{item.label}</span>
            </button>
          ))}
        </nav>
        <div className="p-6 border-t border-madrasa-800 mt-auto text-center">
            <span className="text-[10px] text-madrasa-500 font-bold uppercase tracking-widest">Madrasa OS v2.9.8 | Managed</span>
        </div>
      </aside>
